"""Force-only terrain environment — same as force-only env but with terrain curriculum.

Based on ``go2_force_only_env_cfg.py`` with changes:
1. **Terrain curriculum enabled** — terrain_type="generator" with terrain_levels_vel.
2. **Stairs scaled for Go2** — max step height capped at 15cm (default 23cm too tall for blind).
3. **Reduced flat_orientation_l2** — weight -1.0 (was -2.5) to allow stair-climbing pitch.
4. **Force estimator disabled initially** — threshold 9999.0 so user can evaluate stair-only first.
...
class RobotSceneCfg(InteractiveSceneCfg):
    """Configuration for the terrain scene with Go2 — uses generated terrain."""
...
class ObservationsCfg:
    """Observation specifications — expanded policy obs with dynamics signals."""
...
@configclass
class EventCfg:
    """Events — persistent XY force at fixed 20N."""
    ...
    # interval — persistent XY force (starts at 0, runner activates once walking is stable)
    persistent_xy_force = EventTerm(
        ...
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base"),
            "force_range": (0.0, 0.0),  # runner sets to (0, max_force) when tracking reward threshold met
        },
    )
...
@configclass
class RewardsCfg:
    """Reward terms — same as force-only but with reduced flat_orientation_l2 for stairs."""
    ...
    # Reduced from -2.5 to -1.0: on stairs the robot naturally pitches
...
@configclass
class UnitreeGo2ForceOnlyTerrainEnvCfg(ManagerBasedRLEnvCfg):
    """Go2 locomotion env with terrain curriculum and persistent XY forces for force estimation."""
    ...
    def __post_init__(self):
        ...
        if self.scene.terrain.terrain_generator is not None:
            # Scale terrain for Go2
            self.scene.terrain.terrain_generator.sub_terrains["boxes"].grid_height_range = (0.025, 0.1)
            self.scene.terrain.terrain_generator.sub_terrains["random_rough"].noise_range = (0.01, 0.06)
            self.scene.terrain.terrain_generator.sub_terrains["random_rough"].noise_step = 0.01
            # Scale stairs for Go2 (standing ~40cm; cap max step at 15cm for blind loco)
