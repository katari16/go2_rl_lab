"""Compliant locomotion environment config — trainable estimator + linear mapping.

Based on ``go2_force_only_env_cfg.py`` with these changes:

1. **ForceEstimateObsTerm** (passthrough) in policy and critic obs → 63 / 68 dims.
2. **compliant_track_lin_vel_xy_exp** — linear mapping v* = v_cmd + k(f)*f_hat.
3. **CompliantOnPolicyRunner** handles estimator training and 3-phase gating.

Three training phases (all managed by the runner):
- Phase 1: Walking (forces=0, estimator starts from checkpoint and trains)
- Phase 2 (reward>30): Forces activate, estimator keeps training
- Phase 3 (angular error<7°): Linear mapping activates with piecewise k

Policy obs layout (63 dims)::
    ...
    [61:63] force_estimate (frozen estimator)
...
Critic obs layout (68 dims)::
    ...
    [66:68] force_estimate (frozen estimator)
"""
...
##
# Scene definition (same as force-only)
##
...
class ObservationsCfg:
    """Observation specifications — policy 63 dims, critic 68 dims."""

    @configclass
    class PolicyCfg(ObsGroup):
        """Policy observations: proprioceptive + dynamics + force estimate (63 dims)."""
...
class RewardsCfg:
    ...
    Uses compliant_track_lin_vel_xy_exp which applies linear mapping
    v* = v_cmd + k(f)*f_hat when the runner activates mapping.
    Before activation: identical to standard tracking.
    """
    ...
    # ── Auxiliary rewards (same as force-only) ──────────────────────────
...
@configclass
class UnitreeGo2CompliantEnvCfg(ManagerBasedRLEnvCfg):
    """Go2 compliant locomotion env — trainable estimator + linear compliance mapping."""
