...
class CurriculumCfg:
    ...
    temporal_stages = CurrTerm(
        ...
        params={
            "walking_duration": 2.0,        # seconds before push (if performance is good)
            "recovery_duration": 1.0,       # seconds with frozen tracking rewards
            "post_recovery_duration": 1.0,  # seconds to return to normal walking
            "reward_threshold": 0.85,        # must achieve 85% of max tracking reward to trigger push
            "push_velocity_range": {
                "x": (-1.0, 1.0),            # push velocity range in m/s
                "y": (-1.0, 1.0),
            },
        },
    )
