...
@configclass
class UnitreeActuatorCfg_Go2PACE_April14(PaceDCMotorCfg):
    """Go2 PACE actuator identified in April 14 run (26_04_14_09-41-22, params 199).

    Identified with low PD gains (Kp=8, Kd=0.4). Delay param ~3.50 steps.
    Encoder bias was free during optimization; several entries are saturated
    near the 0.1 rad bound.
    """
    ...
    effort_limit = 23.7
    velocity_limit = 30.0
    max_delay = 4  # identified delay ~3.50 steps

    # --- IDENTIFIED PHYSICAL PARAMETERS (run 26_04_14_09-41-22) ---
    # PACE output order: FR, FL, RR, RL (hip, thigh, calf each)
    armature = {
        "FR_hip_joint": 0.0131, "FR_thigh_joint": 0.0091, "FR_calf_joint": 0.0210,
        "FL_hip_joint": 0.0097, "FL_thigh_joint": 0.0112, "FL_calf_joint": 0.0223,
        "RR_hip_joint": 0.0070, "RR_thigh_joint": 0.0069, "RR_calf_joint": 0.0292,
        "RL_hip_joint": 0.0028, "RL_thigh_joint": 0.0077, "RL_calf_joint": 0.0291,
    }
    viscous_friction = {
        "FR_hip_joint": 0.3298, "FR_thigh_joint": 0.2323, "FR_calf_joint": 0.5302,
        "FL_hip_joint": 0.2687, "FL_thigh_joint": 0.1597, "FL_calf_joint": 1.6849,
        "RR_hip_joint": 0.2541, "RR_thigh_joint": 0.1792, "RR_calf_joint": 0.6794,
        "RL_hip_joint": 0.3009, "RL_thigh_joint": 0.1920, "RL_calf_joint": 0.3451,
    }
    friction = {
        "FR_hip_joint": 0.0271, "FR_thigh_joint": 0.0264, "FR_calf_joint": 0.0103,
        "FL_hip_joint": 0.0274, "FL_thigh_joint": 0.0336, "FL_calf_joint": 0.0173,
        "RR_hip_joint": 0.0271, "RR_thigh_joint": 0.1229, "RR_calf_joint": 0.0146,
        "RL_hip_joint": 0.0791, "RL_thigh_joint": 0.0304, "RL_calf_joint": 0.0182,
    }
    # encoder bias in URDF joint order: FL, FR, RL, RR
    encoder_bias = [
        0.0999, 0.1000, 0.0999,   # FL (PACE indices 3,4,5)
        0.0264, 0.1000, 0.0999,   # FR (PACE indices 0,1,2)
        0.0252, 0.1000, 0.0999,   # RL (PACE indices 9,10,11)
        0.0035, 0.0857, 0.0996,   # RR (PACE indices 6,7,8)
    ]