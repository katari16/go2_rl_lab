...
class Controller():
    def __init__(self, config: Config) -> None:
        ...
        self.obs = np.zeros(config.obs_size_single, dtype=np.float32)
        self.obs_history = np.zeros((config.history_length, config.obs_size_single), dtype=np.float32)
        self.debug_log = []
    ...
    def run(self):
        ...
        # Foot forces (kept for logging only, NOT part of this policy's observations)                                  #
        ...
        dqj_obs = dqj_obs                                                                                              #
                                                                                                                       #
        # The number of actions (12 for the Go2)                                                                       #
        ...
        ########################################## Neural Network inputs (IsaacLab proprioceptive obs order) ############
        # Single frame (45 dims): base_ang_vel(3)*0.25, projected_gravity(3), velocity_commands(3),                    #
        ...
        self.obs[9 : 9 + num_actions] = qj_obs * self.config.dof_pos_scale                                               #
        self.obs[9 + num_actions : 9 + num_actions * 2] = dqj_obs * self.config.dof_vel_scale                          #
        ...
        obs_tensor = torch.from_numpy(                                     #
            self.obs_history.flatten()).unsqueeze(0)                        #
        ...
        self.L_base_vel_cmd_input_1.append(self.obs[6])                             # cmd vx
        self.L_base_vel_cmd_input_2.append(self.obs[7])                             # cmd vy
        self.L_base_vel_cmd_input_3.append(self.obs[8])                             # cmd wz
                                                                                    #
        self.L_base_lin_vel_input_1.append(vx*2)                                    #
        self.L_base_lin_vel_input_2.append(vx*2)                                    #
        self.L_base_lin_vel_input_3.append(vx*2)                                    #
                                                                                    #
        self.L_base_ang_vel_input_1.append(self.obs[0])                             # ang_vel x
        self.L_base_ang_vel_input_2.append(self.obs[1])                             # ang_vel y
        self.L_base_ang_vel_input_3.append(self.obs[2])                             # ang_vel z
        #############################################################################
...
if __name__ == "__main__":
    ...
    ############### Save deployment logs to deploy_real/logs/ #############################
    log_dir = current_dir.parent / "logs"                                                 #
    log_dir.mkdir(parents=True, exist_ok=True)                                            #
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")                              #
    log_path = log_dir / f"deploy_{timestamp}.npz"                                        #
                                                                                          #
    N = len(controller.debug_log)                                                         #
    if N > 0:                                                                             #
        observations    = np.array([s['obs']               for s in controller.debug_log])#
        actions         = np.array([s['action']            for s in controller.debug_log])#
        target_dof_pos  = np.array([s['target_dof_pos']    for s in controller.debug_log])#
        joint_pos_isaac = np.array([s['joint_pos_isaac']   for s in controller.debug_log])#
        joint_vel_isaac = np.array([s['joint_vel_isaac']   for s in controller.debug_log])#
        joint_pos_raw   = np.array([s['joint_pos_raw']     for s in controller.debug_log])#
        joint_vel_raw   = np.array([s['joint_vel_raw']     for s in controller.debug_log])#
        velocity_cmd    = np.array([s['velocity_cmd']      for s in controller.debug_log])#
        imu_quat        = np.array([s['imu_quat']          for s in controller.debug_log])#
        imu_gyro        = np.array([s['imu_gyro']          for s in controller.debug_log])#
        imu_rpy         = np.array([s['imu_rpy']           for s in controller.debug_log])#
        projected_grav  = np.array([s['projected_gravity']  for s in controller.debug_log])#
        foot_forces     = np.array([s['foot_forces']       for s in controller.debug_log])#
        base_lin_vel_kin= np.array([s['base_lin_vel_kinematics'] for s in controller.debug_log])#
        steps           = np.array([s['step']              for s in controller.debug_log])#
        timestamps      = steps * config.control_dt                                       #
                                                                                          #
        np.savez(log_path,                                                                #
            ...
            base_lin_vel_kinematics=base_lin_vel_kin,                                     #
            steps=steps,                                                                  #
