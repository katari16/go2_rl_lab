...
def find_latest_per_run(recordings_dir: Path) -> dict:
    """Return {stem: latest_csv_path} for each config stem."""
    ...
    return {stem: sorted(files)[-1][1] for stem, files in by_stem.items()}
...
def main():
    ...
    latest = find_latest_per_run(RECORDINGS_DIR)
    if not latest:
    ...
    for idx, (stem, csv_path) in enumerate(sorted(latest.items())):
        df = pd.read_csv(csv_path)
        t = df["t"].values
        mag = df["force_mag"].values
        mask = t >= 5.0
        t, mag = t[mask], mag[mask]
        t = t - t[0]  # re-zero time

        mae = np.mean(np.abs(mag - args.gt))
        label = LABELS.get(stem, stem)
        label = f"{label}  |  MAE={mae:.2f}N"

        ax.plot(t, mag, color=colors[idx % len(colors)], linewidth=1.2, alpha=0.85, label=label)
    ...
    print(f"\n{'Run':<45} {'Steps':>6}  {'MAE (N)':>8}  {'Mean (N)':>9}  {'Std (N)':>8}")
    print("-" * 80)
    for stem, csv_path in sorted(latest.items()):
        df = pd.read_csv(csv_path)
        mag = df["force_mag"].values[df["t"].values >= 5.0]
        mae = np.mean(np.abs(mag - args.gt))
        print(f"{LABELS.get(stem, stem):<45} {len(mag):>6}  {mae:>8.2f}  {mag.mean():>9.2f}  {mag.std():>8.2f}")
