...
Copy of deploy_compliant_no_foot_xyz.py with optional --sim-visual flag that streams
robot state via UDP to a standalone MuJoCo viewer (real_viz/real_mujoco_viz.py).

Usage:
    python deploy_real/deploy_compliant_no_foot_xyz_sim_visual.py enp3s0 go2_compliant_no_foot_xyz.yaml
    python deploy_real/deploy_compliant_no_foot_xyz_sim_visual.py enp3s0 go2_compliant_no_foot_xyz.yaml --sim-visual
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sim-visual", action="store_true", default=False,
                        help="Stream robot state via UDP for MuJoCo visualization.")
    ...
    # ── Sim-visual UDP setup ──────────────────────────────────────────────
    SIM_VIZ_PORT = 9871
    SIM_VIZ_ADDR = ("127.0.0.1", SIM_VIZ_PORT)
    viz_sock = None
    if args.sim_visual:
