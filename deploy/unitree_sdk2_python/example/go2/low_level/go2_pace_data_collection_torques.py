...
class Custom:
    def __init__(self):
        ...
        self.des_pos_buffer = []
        self.dof_vel_buffer = []
        self.tau_est_buffer = []  # measured torque from motors
        self.tau_cmd_buffer = []  # calculated PD torque
    ...
    def LowCmdWrite(self):
        ...
        if (self.percent_3 == 1.0) and (self.trajectory_counter < self.num_steps):
            ...
            actual_dq = [self.low_state.motor_state[i].dq for i in range(12)]
            tau_est = [self.low_state.motor_state[i].tau_est for i in range(12)]

            # Calculate commanded torque: tau_cmd = Kp * (q_des - q) - Kd * dq
            tau_cmd = [
                self.Kp * (desired_q[i] - actual_q[i]) - self.Kd * actual_dq[i]
                for i in range(12)
            ]
            ...
            self.dof_vel_buffer.append(actual_dq)
            self.tau_est_buffer.append(tau_est)
            self.tau_cmd_buffer.append(tau_cmd)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        if custom.percent_sine >= 1.0:
            time.sleep(1)
            print("\nSine Signal Complete. Saving PACE data with torques...")

            # Create dated folder (day_month_data_torques format)
            date_folder = datetime.now().strftime("%d_%m_data_torques")
            ...
            torch.save({
                ...
                "dof_vel": torch.tensor(custom.dof_vel_buffer, dtype=torch.float32),
                "tau_est": torch.tensor(custom.tau_est_buffer, dtype=torch.float32),
                "tau_cmd": torch.tensor(custom.tau_cmd_buffer, dtype=torch.float32),
            }, filepath)
